...
// $Id: ASTModel.java 5494 2015-02-05 12:59:25Z lhamann $
...
public class ASTModel extends ASTAnnotatable {
    ...
    public MModel gen(Context ctx) {
        ...
        while(cIt.hasNext()) {
            ...
            try {
				...
				cls.setAnnotationTag(c.getAnnotationTag());
                model.addClass(cls);
            } catch (SemanticException ex) {
                ctx.reportError(ex);
                cIt.remove();
	...
	public void UpdateWeightsThroughInheritanceTree(MClass cls){
		Set<MClassImpl> classes = (Set<MClassImpl>) cls.allChildren();
		
		if (cls.getAnnotationTag()==null) return;
		
		AnnotationTag t1 = cls.getAnnotationTag();
		for (MClass c : classes){
			AnnotationTag t2 = c.getAnnotationTag();
			if (c.getAnnotationTag()!=null){
				/*if (t1.getWeight().compareTo(t2.getWeight())==1){
					throw new WeightException("Error: a subclass cannot have a weight less than its parent. ");
				}*/
			}
			else{
				c.setAnnotationTag(t1);
			}
		}
	}
}
