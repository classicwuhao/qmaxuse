...
public interface AbstractRankVisitor{
	...
	int visitExists (ExpExists exp);
	int visitForAll (ExpForAll exp);
	...
	int visitNavigation (ExpNavigation exp);
	...
	int visitVariable (ExpVariable exp);
	//void visitClosure(ExpClosure exp);
	//void visitOclInState(ExpOclInState exp);
	int visitVarDeclList(VarDeclList varDeclList);
	int visitVarDecl(VarDecl varDecl);
